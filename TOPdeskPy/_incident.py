...
class incident:
    ...
    def get(self, incident):
        if self.utils.is_valid_uuid(incident):
            return self.utils.handle_topdesk_response(
                self.utils.request_topdesk("/tas/api/incidents/id/{}".format(incident)))
        else:
            return self.utils.handle_topdesk_response(
                self.utils.request_topdesk("/tas/api/incidents/number/{}".format(incident)))
    ...
    def deescalate(self, incident, reason_id=None):
        ...
        if self.utils.is_valid_uuid(incident):
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/id/{}/deescalate".format(incident), param))
        else:
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/number/{}/deescalate".format(incident), param))
    ...
    def escalate(self, incident, reason_id=None):
        ...
        if self.utils.is_valid_uuid(incident):
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/id/{}/escalate".format(incident), param))
        else:
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/number/{}/escalate".format(incident), param))
    ...
    def get_progress_trail(self, incident, inlineimages=False, non_api_attachments_url=False, page_size=100):
        ...
        if self.utils.is_valid_uuid(incident):
            return self.utils.handle_topdesk_response(
                self.utils.request_topdesk("/tas/api/incidents/id/{}/progresstrail".format(incident),
                                           page_size=page_size, extended_uri=ext_uri))
        else:
            return self.utils.handle_topdesk_response(
                self.utils.request_topdesk("/tas/api/incidents/number/{}/progresstrail".format(incident),
                                           page_size=page_size, extended_uri=ext_uri))
    ...
    class _action:
        ...
        def get_list(self, incident, inlineimages=False, non_api_attachments_url=False, page_size=100):
            ...
            if self.utils.is_valid_uuid(incident):
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/id/{}/actions".format(incident), page_size=page_size,
                                               extended_uri=ext_uri))
            else:
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/number/{}/actions".format(incident),
                                               page_size=page_size, extended_uri=ext_uri))
        ...
        def get(self, incident, actions_id, inlineimages=False, non_api_attachments_url=False):
            ...
            if self.utils.is_valid_uuid(incident):
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/id/{}/actions/{}".format(incident, actions_id),
                                               page_size=10, extended_uri=ext_uri))
            else:
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/number/{}/actions/{}".format(incident, actions_id),
                                               page_size=10, extended_uri=ext_uri))

        def delete(self, incident, actions_id):
            if self.utils.is_valid_uuid(incident):
                return self.utils.handle_topdesk_response(
                    self.utils.delete_from_topdesk("/tas/api/incidents/id/{}/actions/{}".format(incident, actions_id),
                                                   None))
            else:
                return self.utils.handle_topdesk_response(self.utils.delete_from_topdesk(
                    "/tas/api/incidents/number/{}/actions/{}".format(incident, actions_id), None))
    ...
    class _request:
        ...
        def get_list(self, incident, inlineimages=False, non_api_attachments_url=False, page_size=100):
            ...
            if self.utils.is_valid_uuid(incident):
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/id/{}/requests".format(incident),
                                               page_size=page_size, extended_uri=ext_uri))
            else:
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/number/{}/requests".format(incident),
                                               page_size=page_size, extended_uri=ext_uri))
        ...
        def get(self, incident, request_id, inlineimages=False, non_api_attachments_url=False):
            ...
            if self.utils.is_valid_uuid(incident):
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/id/{}/actions/{}".format(incident, request_id),
                                               page_size=10, extended_uri=ext_uri))
            else:
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/number/{}/actions/{}".format(incident, request_id),
                                               page_size=10, extended_uri=ext_uri))

        def delete(self, incident, request_id):
            if self.utils.is_valid_uuid(incident):
                return self.utils.handle_topdesk_response(
                    self.utils.delete_from_topdesk("/tas/api/incidents/id/{}/actions/{}".format(incident, request_id),
                                                   None))
            else:
                return self.utils.handle_topdesk_response(self.utils.delete_from_topdesk(
                    "/tas/api/incidents/number/{}/actions/{}".format(incident, request_id), None))
    ...
    class _timespent:
        ...
        def get(self, incident):
            if self.utils.is_valid_uuid(incident):
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/id/{}/timespent".format(incident)))
            else:
                return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk("/tas/api/incidents/number/{}/timespent".format(incident)))
        ...
        def register(self, incident, timespent, **kwargs):
            ...
            if self.utils.is_valid_uuid(incident):
                return self.utils.handle_topdesk_response(
                    self.utils.post_to_topdesk("/tas/api/incidents/id/{}/timespent".format(incident), param))
            else:
                return self.utils.handle_topdesk_response(
                    self.utils.post_to_topdesk("/tas/api/incidents/number/{}/timespent".format(incident), param))
    ...
    def create(self, caller, **kwargs):
        ...
        return self.utils.handle_topdesk_response(
            self.utils.post_to_topdesk("/tas/api/incidents/", self.utils.add_id_jsonbody(**create_body)))

    def update(self, incident, **kwargs):
        if self.utils.is_valid_uuid(incident):
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/id/{}".format(incident),
                                          self.utils.add_id_jsonbody(**kwargs)))
        else:
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/number/{}".format(incident),
                                          self.utils.add_id_jsonbody(**kwargs)))
    ...
    def archive(self, incident, reason_id=None):
        ...
        if self.utils.is_valid_uuid(incident):
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/id/{}/archive".format(incident), param))
        else:
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/number/{}/archive".format(incident), param))

    def unarchive(self, incident):
        if self.utils.is_valid_uuid(incident):
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/id/{}/unarchive".format(incident), None))
        else:
            return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk("/tas/api/incidents/number/{}/unarchive".format(incident), None))

    def get_list(self, archived=False, page_size=100, **kwargs):
        return self.utils.handle_topdesk_response(
            self.utils.request_topdesk("/tas/api/incidents/", archived, page_size=page_size, extended_uri=kwargs))
